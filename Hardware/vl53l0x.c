...
void VL53L0X_WriteByte(uint8_t add,uint8_t reg,uint8_t data)
{
	//HAL_I2C_Mem_Write(&hi2c1 ,(add<<1)|0,reg,I2C_MEMADD_SIZE_8BIT,&data,1,0xffff);
	 App_I2C_Start();
	 App_I2C_WriteByte(add<<1);
	 App_I2C_ReceiveAck();
	 App_I2C_WriteByte(reg);
	 App_I2C_ReceiveAck();
...
void VL53L0X_WriteByte_16Bit(uint8_t add,uint8_t reg,uint16_t data)
{
	...
	App_I2C_WriteByte(reg);
	App_I2C_ReceiveAck();
	App_I2C_WriteByte(data2[0]);
	App_I2C_ReceiveAck();
	App_I2C_WriteByte(data2[1]);
	App_I2C_ReceiveAck();
	App_I2C_Stop();
}
...
void VL53L0X_WriteByte_32Bit(uint8_t add,uint8_t reg,uint32_t data)
{
	...
	App_I2C_WriteByte(data2[1]);
	App_I2C_ReceiveAck();
	App_I2C_WriteByte(data2[2]);
	App_I2C_ReceiveAck();
	App_I2C_WriteByte(data2[3]);
	App_I2C_ReceiveAck();
	App_I2C_Stop();
}
...
uint16_t VL53L0X_ReadBytee_16Bit(uint8_t add,uint16_t reg)
{
	...
	App_I2C_WriteByte(add<<1|1);
	App_I2C_ReceiveAck();
	data2[0] = App_I2C_ReadByte();
	App_I2C_SendAck(0);
	data2[1] = App_I2C_ReadByte();
	App_I2C_SendAck(1);
	App_I2C_Stop();
...
void VL53L0X_readMulti(uint8_t add,uint8_t reg,  uint8_t * dst, uint8_t count)
{
	...
	{
			dst[i] = App_I2C_ReadByte();
		  App_I2C_SendAck(0);
	}
	dst[count-1] = App_I2C_ReadByte();
	App_I2C_SendAck(1);
	//HAL_I2C_Mem_Read(&hi2c1 ,(add<<1)|1,reg,I2C_MEMADD_SIZE_8BIT,dst,count,0xffff);
}
...
{
		//HAL_I2C_Mem_Write(&hi2c1 ,(add<<1)|0,reg,I2C_MEMADD_SIZE_8BIT,src,count,0xffff);
	App_I2C_Start();
	App_I2C_WriteByte(add<<1);
	App_I2C_ReceiveAck();
	App_I2C_WriteByte(reg);
